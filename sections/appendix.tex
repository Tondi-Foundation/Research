...
Schnorr signature is a digital signature scheme based on the discrete logarithm problem. Given elliptic curve group $(G, g, n)$, private key $x \in \mathbb{Z}_n$, public key $P = x \cdot g$, the signing process for message $m$ is:
\begin{enumerate}
    \item Choose random number $k$, compute $R = k \cdot g$
    \item Compute $e = H(R \| P \| m)$
