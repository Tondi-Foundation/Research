...
For a payment $p$ routed through CSP set $\mathcal{H}$, the anonymity set size is defined as the Cartesian product of channel sets:
$$ |\mathcal{AS}(p)| = \prod_{h \in \mathcal{H}} |\mathrm{Channels}_h| $$
Payment $p$ is \textbf{$k$-anonymous} iff $|\mathcal{AS}(p)| \geq k$.
\end{definition}
...
$$ T_{\mathrm{latency}} = T_{\mathrm{base}} + \alpha \cdot \log |\mathcal{AS}| $$
\end{theorem}
...
\begin{tabularx}{\linewidth}{@{}lXl@{}}
\toprule
\textbf{Mode} & \textbf{Latency} & \textbf{Anonymity Set} \\
\midrule
