...
The probability of a successful rollback is bounded by:
$$ P_{\mathrm{rollback}} \le P_{\mathrm{51\%}} \times P_{\mathrm{victim\_offline}} $$
\end{theorem}
...
\end{equation}
where $d$ is the topology depth and $\alpha \approx 0.1$ is the depth penalty. Unpaid rent can be claimed by any searcher via a \textbf{Merge Transaction}, incentivizing state pruning.
...
Under STPC, the expected confirmation time for the highest state is bounded:
$$ E[\mathrm{Confirm}] \le \frac{1}{\lambda_{\mathrm{block}}} \cdot (1 + \epsilon_{\mathrm{jitter}}) $$
\end{theorem}
