...
\end{definition}
where $\rho$ is the risk aversion coefficient ($\approx 0.5 \text{--} 2.0$) and $C_{\mathrm{opp}}$ represents DeFi opportunity cost.

\begin{theorem}[Competitive Equilibrium]
In a market with $N \ge 3$ CSPs and free entry:
$$ \lim_{t \to \infty} \Fee_{\mathrm{CSP}_i} \to \Cmarg + \epsilon $$
\end{theorem}
\begin{proof}
...
Since PTLCs cryptographically prevent theft ($P(\text{Success}|\text{Steal}) = 0$) and the L1 fallback option bounds the ``Delay'' utility ($U_{\text{delay}} < \text{ReputationCost}$), we have $U_{\text{honest}} > U_{\text{delay}} > U_{\text{steal}}$. Thus, Honest is the Nash Equilibrium.
\end{proof}
