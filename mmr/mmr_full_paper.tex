...
\begin{tikzpicture}[
    ...
    \node[draw, rounded corners=5pt, fill=gray!5, minimum width=6cm, minimum height=1cm] at (5,-2.5) {};
    \node[selected, scale=0.5, minimum width=0.6cm] at (2.5,-2.5) {};
    \node[right, font=\footnotesize] at (3,-2.5) {Selected Parent Chain};
    \node[block, scale=0.5, minimum width=0.6cm] at (6,-2.5) {};
    \node[right, font=\footnotesize] at (6.5,-2.5) {Mergeset Blocks};
\end{tikzpicture}
\caption{Kaspa BlockDAG Structure. The green blocks form the \emph{selected parent chain} (the canonical backbone determined by GHOSTDAG). Blue blocks are \emph{mergeset blocks}---parallel blocks whose transactions are merged into the ledger when a selected-chain block references them as additional parents.}
\label{fig:blockdag}
\end{figure}
...
\subsubsection{Integration with Kaspa's Pruning Mechanism}
...
\begin{itemize}
    \item \textbf{Pre-pruning proof generation:} Nodes can automatically generate and cache inclusion proofs for their own transactions before pruning
    \item \textbf{Checkpoint alignment:} MMR checkpoints can align with Kaspa's posterity headers (every $\sim$86,400 blocks)
    \item \textbf{Archive incentives:} Archive nodes maintaining full MMR history can charge for historical proof generation, creating a sustainable ecosystem
\end{itemize}
...
\begin{tikzpicture}[
    ...
    \draw[arrow, dashed, gray!50, line width=0.8pt] (kaspa.south) -- ++(0,-0.6) -| (mmr.west);
    ...
    % Step 1: Lock funds on Kaspa
    \draw[arrow, blue!60, line width=1.2pt] (kaspa.north) -- ++(0,0.8) -| (user.west);
    \node[circle, fill=blue!20, draw=blue!50, font=\tiny\bfseries, inner sep=2pt] at (1,1.8) {1};
    \node[note, text=blue!60] at (1,1.4) {Lock funds};
