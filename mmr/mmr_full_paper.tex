...
% ============ Document Info ============
\title{Logarithmic Sequencing Commitments for GHOSTDAG BlockDAGs\\via Merkle Mountain Ranges\\
...
% ============ Abstract ============
\begin{abstract}
We propose a Merkle Mountain Range (MMR) based \emph{Execution Log} scheme for GHOSTDAG-style blockDAG protocols that output a deterministic total order of accepted transactions. Unlike auxiliary history logs, our approach treats the MMR as a \textbf{first-class consensus artifact}---the \code{exec\_root} (Merkle root of accepted transaction IDs) and \code{elog\_root} (execution log accumulator root) are \emph{outputs of consensus execution itself}, validated alongside UTXO commitments. A block with incorrect execution log fields is invalid at the consensus level, not merely inconsistent with an external index.

We formalize the \textbf{Consensus API Contract}: GHOSTDAG implementations must expose a deterministic \code{AcceptedTxList(B)} for each block $B$, representing the exact sequence of transaction IDs accepted during UTXO validation. The \code{exec\_root} commits to this sequence, and the MMR leaf binds \code{exec\_root} to the block's selected parent hash, creating an append-only execution log. This design ensures that the commitment reflects the \emph{actual execution order}---not merely block ordering. Layer-2 systems can prove that a transaction was executed at position $(j, i)$: block $j$ in the selected chain, transaction $i$ within that block's accepted list.

We analyze the overhead based on experiments with Tondi, a GHOSTDAG implementation running at 10 blocks per second. The scheme adds 40 bytes per header (negligible bandwidth, $<0.1\%$) and moderate storage (tens of GB/year for archive nodes). In return, the system gains $\bigO(\log n)$ execution proofs: verifying a transaction from 100,000 blocks ago requires $\sim$17 hashes ($<$1~KB), versus 100,000 hashes in linear schemes. We present use cases including trustless cross-chain bridges, fast node bootstrapping via skip proofs, and verifiable disaster recovery.

\textbf{Keywords:} Merkle Mountain Range, GHOSTDAG, Execution Log, Consensus Commitment, Logarithmic Proofs, Layer-2 Scaling, Light Client, Cross-Chain Bridge
\end{abstract}
...
\textbf{Terminology:} We distinguish between \emph{BlockDAG} as a structural paradigm (blocks forming a DAG rather than a chain, allowing parallel block production) and \emph{GHOSTDAG} as a specific consensus algorithm that operates on BlockDAGs. GHOSTDAG provides rules for ordering blocks (via blue scores), selecting a canonical chain (the selected parent chain), and merging parallel blocks into a deterministic execution sequence. Our scheme applies specifically to \textbf{GHOSTDAG-style protocols}---those that output a deterministic total order of accepted transactions via selected-parent-chain + mergeset ordering.

GHOSTDAG-based blockDAGs are high-throughput protocols where blocks are produced in parallel (multi-leader consensus). Rather than a single chain of blocks, GHOSTDAG orders blocks in a DAG, identifying a \emph{selected parent chain} (analogous to a backbone chain) and merging other parallel blocks (called the mergeset) into the ledger. In such a system, determining a global transaction order is non-trivial: multiple blocks (produced at nearly the same time) may contain different transactions, and the protocol must decide a consistent inclusion order for all transactions.
...
\end{tikzpicture}
\caption{GHOSTDAG BlockDAG Structure. The green blocks form the \emph{selected parent chain} (the canonical backbone determined by GHOSTDAG). Blue blocks are \emph{mergeset blocks}---parallel blocks whose transactions are merged into the ledger when a selected-chain block references them as additional parents.}
\label{fig:blockdag}
\end{figure}

Two key challenges arise in GHOSTDAG-based blockDAGs:

\textbf{Sequencing Commitment:} How to commit each block to the global canonical order of all transactions up to that point, so that Layer-2 (L2) systems can trustlessly follow the exact L1 transaction sequence. A straightforward approach is adding a chained commitment in each block header that hashes together the previous commitment and the Merkle root of the current block's accepted transactions: $\text{SC}_n = H(\text{SC}_{n-1} \parallel \text{ExecRoot}_n)$. While simple, a purely linear commitment implies that to prove a transaction from many blocks ago, one might need to traverse or reveal a long chain of commitments---a verification cost linear in the distance between blocks, which is prohibitive for L2 verification if the distance is large.

\textbf{Logarithmic Inclusion Proofs:} How to enable compact proofs (of size $\bigO(\log N)$) that a given transaction was included in the ledger by a certain block, without requiring a client to download or trust the entire chain history. Naive proofs of inclusion in any blockDAG could be linear in the distance from the block to be verified. The challenge is to use a data structure that enables skip-ahead verification, reminiscent of the FlyClient approach for light clients~\cite{flyclient}.
...
Our approach, which we call the \emph{Execution-MMR Sequencing Commitment}, integrates with GHOSTDAG consensus as follows:
\begin{itemize}
...
This design effectively provides \textbf{everlasting cryptographic execution receipts} for transactions: a light client or an L2 contract can verify that a transaction was executed at position $(j, i)$---block $j$ in the selected chain, transaction $i$ within that block---by checking a short proof against a recent block header's execution log root. The proof size and verification cost grow only logarithmically with the age of the transaction, rather than linearly as in a simple chained commitment scheme.

In particular, our contributions and findings include:
\begin{enumerate}
    \item \textbf{Execution-Level Commitment Scheme:} We design a consensus change that introduces \code{exec\_root} and \code{elog\_root} as \emph{consensus execution outputs}. Unlike auxiliary logs, these commitments are validated as part of block acceptance---a wrong \code{exec\_root} or \code{elog\_root} causes block rejection, just like a wrong UTXO commitment.
    
    \item \textbf{Formalization of Canonical Execution Order:} We define $\TxList(B)$ as the exact sequence of accepted transaction IDs produced by \code{calculate\_utxo\_state()}. This is not a specification we impose---it is the existing deterministic behavior of GHOSTDAG consensus. We simply make it externally verifiable.
    
    \item \textbf{Unified Ordering and Proof Mechanism:} The \code{exec\_root} achieves the ordering commitment goal. The \code{elog\_root} achieves the logarithmic proof goal. Together, they provide a single, unified mechanism for both transaction ordering and efficient historical proofs.
    
    \item \textbf{Two-Layer Proof Model:} We formalize the proof structure as:
    \begin{itemize}
        \item \textbf{Intra-block execution proof:} Transaction $\to$ \code{exec\_root} (proves execution index within block)
        \item \textbf{Inter-block MMR proof:} Block leaf $\to$ \code{elog\_root} (proves block's position in execution history)
    \end{itemize}
    Together, these prove the transaction's global execution position with $\bigO(\log n)$ complexity.
...
The blockDAG architecture complicates transaction ordering in two ways:
\begin{enumerate}
...
For a block $B$:
\[
\mathsf{ExecRoot}(B) = \mathsf{MerkleRoot}(\mathsf{TxList}(B))
\]
where $\mathsf{MerkleRoot}$ is a standard binary Merkle tree over the ordered list of transaction IDs.
\end{definition}
...
\begin{lstlisting}[language=Python, caption={Consensus API Contract binding}]
...
\end{tikzpicture}
\caption{Construction of the canonical transaction list ($\TxList$) and $\ExecRoot$ for block $B$. Transactions are ordered deterministically: selected parent first, then mergeset blocks in consensus order, finally block $B$'s own transactions.}
...
\begin{equation}
\TxList(B) := \text{ctx.accepted\_tx\_ids} \quad \text{(from \code{UtxoProcessingContext})}
\end{equation}
...
\textbf{Leaf Uniqueness:} The pair $(h_{P_{\text{sel}}(B)}, \ExecRoot(B))$ uniquely identifies a block's execution commitment. Two blocks with the same selected parent but different accepted transactions produce different leaves. Two blocks with different parents produce different leaves regardless of transactions.
...
\begin{itemize}
    \item \textbf{Hash function:} BLAKE3-256 (or SHA-256 for compatibility)
    \item \textbf{Domain prefixes:} ASCII-encoded, null-terminated, padded to 16 bytes
...
def compute_leaf(parent_hash: bytes, exec_root: bytes) -> bytes:
    domain = b"ELOG_LEAF_V1\x00\x00\x00\x00"  # 16 bytes
    preimage = domain + parent_hash + exec_root  # 16 + 32 + 32 = 80 bytes
...
\textbf{Domain Separation Constants:} To prevent hash collisions across different contexts, we define fixed domain prefixes:
\begin{itemize}
    \item \code{DOMAIN\_LEAF} = \code{``KASPA\_MMR\_LEAF\_V1''}
    \item \code{DOMAIN\_NODE} = \code{``KASPA\_MMR\_NODE\_V1''}
    \item \code{DOMAIN\_BAG} = \code{``KASPA\_MMR\_BAG\_V1''}
    \item \code{DOMAIN\_ROOT} = \code{``KASPA\_MMR\_ROOT\_V1''}
\end{itemize}
...
\begin{itemize}
    \item Proof $\pi_1$: transaction $\to$ \code{elog\_root} at checkpoint $C_1$
    \item Proof $\pi_2$: checkpoint $C_1$'s state $\to$ \code{elog\_root} at checkpoint $C_2$
\end{itemize}
A verifier holding \code{elog\_root} at $C_2$ can verify the transaction by composing $\pi_1$ and $\pi_2$. This enables a hierarchical proof ecosystem where archive nodes store proof segments and users compose them on demand.
...
MMRProof = {
    ...
    elog_size: u64,              # Size at proof time
    siblings: [Hash],           # Path from leaf to peak
    peaks: [Hash],              # All peak hashes (or peaks_root)
...
\For{p in MMRProof.peaks}
    ...
    \EndIf
\EndFor
\State expected\_root $\gets \Hroot(\text{bag} \parallel \text{encode}_{64}(\text{MMRProof.elog\_size}))$
\State \Return expected\_root $=$ $\text{elog\_root}_T$
\end{algorithmic}
\end{algorithm}
...
\begin{tikzpicture}[
    ...
    \node[chain, fill=blue!15, draw=blue!50] (tondi) at (0,0) {\textbf{Tondi}\\BlockDAG};
...
Our work builds on GHOSTDAG~\cite{ghostdag}, which provides a specific linearization rule for blockDAGs:
...
\begin{itemize}
    \item Can construct arbitrary proofs and data structures
    \item Cannot break cryptographic hash functions (collision resistance assumed)
    \item Cannot rewrite confirmed blockchain headers (would require consensus attack)
\end{itemize}
...
\textbf{Proof Sketch:} The two-layer proof binds transaction $\rightarrow$ \code{exec\_root} $\rightarrow$ MMR leaf $\rightarrow$ \code{elog\_root} $\rightarrow$ block header. Each binding is via cryptographic hashing. Forging any component requires either a hash collision or consensus-level attack.
...
\begin{enumerate}
    ...
    \item \textbf{Hash Function Security:} The hash function (e.g., BLAKE3, SHA-256) must be collision-resistant
...
These are the same assumptions required for trusting any blockchain data, making execution log proofs no weaker than consensus itself.
...
We have presented an \emph{Execution-MMR Sequencing Commitment} scheme for GHOSTDAG-based blockDAGs that fundamentally redefines how external systems interact with consensus. The MMR is not an auxiliary history log, but a \textbf{first-class consensus artifact} that serves as the canonical interface to the blockchain's execution results.
...
\bibitem{linearseq}
Linear Sequencing Commitments for BlockDAG Transaction Ordering.
\textit{BlockDAG Research}, 2024.

\bibitem{pochm}
S. Wyborski,
``Proof of Chain Membership: Logarithmic Transaction Inclusion Proofs,''
\textit{BlockDAG Improvement Proposals}, 2023.
