...
\begin{itemize}
    \item MMR provides $182\times$ faster verification than linear commitment
    \item MMR proof is $3,556\times$ smaller than linear commitment
    \item MMR has higher storage overhead (node store) but enables proof generation
    \item Throughput impact is minimal ($<1\%$) for all approaches
\end{itemize}
...
\textbf{Quantum Security Boundary:} Under quantum attacks, collision resistance degrades from $2^{128}$ to $2^{64}$ (Grover's algorithm). To maintain post-quantum security, one would need to:
\begin{enumerate}
    \item Replace BLAKE3-256 with a post-quantum hash function (e.g., SHA-3-512 or future NIST standards)
    \item Increase hash output size to 512 bits for 256-bit post-quantum security
\end{enumerate}

The MMR structure and proof algorithms remain unchanged; only the underlying hash function and output size change. A detailed post-quantum security analysis is left for future work.
...
\begin{itemize}
    \item \textbf{Single-Shard Assumption:} Our scheme assumes a single-shard architecture. In a sharded system, each shard would maintain an independent MMR, and cross-shard transactions would require coordination through a global commitment layer. Extending our scheme to sharded architectures is an important direction for future work.
...
\begin{itemize}
    \item \textbf{Sharded Architecture Extension:} In a sharded PoW DAG ledger, each shard maintains an independent MMR. Cross-shard transactions require a global coordination layer that commits to the ordering of cross-shard events. We plan to design a \emph{global execution log} that aggregates shard-level MMRs while maintaining logarithmic proof complexity. This would enable L2 systems to verify cross-shard transaction ordering.
    ...
    \begin{enumerate}
        \item The correctness of the MMR append algorithm
        \item The proof verification algorithm's soundness
        \item The reorg handling correctness (invariant preservation)
        \item The security properties (execution-fidelity, reorg-soundness, non-equivocation)
    \end{enumerate}
    This would provide machine-checkable guarantees that the protocol behaves as specified.
