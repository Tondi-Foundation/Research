...
\toprule
\textbf{Feature} & \textbf{Lightning} & \textbf{BIP-118} & \textbf{Native Eltoo} \\
\midrule
...
\begin{equation}
H_{\mathrm{NativeEltoo}}(t) \approx \mathrm{size}(\mathrm{State}_{\mathrm{curr}}) + \mathrm{size}(\mathrm{Fund}) \approx \mathcal{O}(1)
\end{equation}
...
\begin{tikzpicture}[
    ...
    % --- Right: Native Eltoo (Low Entropy) ---
    \node[font=\bfseries] at (6.5, 4.5) {Native Eltoo: $\mathcal{O}(1)$ Entropy};
...
\end{tikzpicture}
\caption{Verification Causality Graph. Lightning validators (left) face linear complexity due to historical dependencies. Native Eltoo validators (right) only verify the latest state against the static Fund anchor.}
...
For Native Eltoo with $H = \mathcal{O}(1)$, only the current state and fund anchor are required. The recovery probability is $(1-p)^{O(1)} \approx 1$ for small $p$, independent of channel history length.
\end{proof}
