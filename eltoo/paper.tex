...
% ============ Document Info ============
\title{Native Eltoo: A Consensus-Embedded Protocol for High-Frequency Payment Networks}
\author{
...
Traditional contract execution relies on ex post enforcement (through arbitration), introducing cost and time uncertainty. The Native Eltoo achieves ex ante enforcement through consensus rules:
\begin{align*}
\text{Traditional Mode: } & \text{Contract} \xrightarrow{\text{Dispute}} \text{Arbitration} \xrightarrow{\text{Judgment}} \text{Enforcement} \\
\text{The Native Eltoo: } & \text{State\_UTXO} \xrightarrow{\tau_{\text{settle}}} \text{Value\_Distribution} \quad (\text{deterministic execution})
\end{align*}
...
Blockchain system security is often described as ``trust minimization.'' The Native Eltoo further pursues \textbf{trust elimination}---making certain types of trust assumptions unnecessary through protocol design:
...
\toprule
\textbf{Trust Assumption} & \textbf{Traditional PCN} & \textbf{The Native Eltoo} & \textbf{Elimination Mechanism} \\
\midrule
