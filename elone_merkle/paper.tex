...
\textbf{Theoretical Results:} We prove that partition conservation implies zero channel-paid fees and that deterministic settlement descriptors yield a unique unilateral settlement output set, up to canonical serialization. We also show that proof-first verification is naturally local in the touched state footprint and that a bare state root is insufficient for unilateral recovery. These results replace fee-cap reasoning with a stronger invariant over value partitions and replace plaintext-authoritative recovery with a sharper notion of recoverable protocol meaning.

\textbf{Engineering Interpretation:} This paper should be read as a March 2026 protocol proposal derived from the new Elone Merkle design memorandum. It is not a claim that the full design is already implemented. Rather, it provides a research-grade specification and migration discipline for upgrading Elone from a plaintext-assisted channel protocol to a proof-first, merkleized channel architecture.
...
\begin{theorem}[Witness Locality]
...
Every Elone transaction is divided into two value partitions:
\begin{itemize}
...
Let $V_C^{in}$ and $V_C^{out}$ denote the total input and output value of the channel partition. Let $V_S^{in}$ and $V_S^{out}$ denote the corresponding values for the sponsor partition. A transaction satisfies \textbf{partition conservation} iff:
\[
V_C^{in} = V_C^{out}
\]
and
...
\begin{theorem}[Channel Fee Isolation]
If partition conservation holds, then channel-owned value never pays transaction fees.
\end{theorem}

\begin{proof}
By definition, $V_C^{in} = V_C^{out}$. Therefore no value deficit is introduced within the channel partition. Since total fee is defined entirely by the sponsor deficit $V_S^{in} - V_S^{out}$, all fees must be paid by the sponsor partition. Hence channel-paid fee is identically zero.
\end{proof}
