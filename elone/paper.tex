...
% ============ Document Info ============
\title{Elone: A Consensus-Embedded Protocol for High-Frequency Payment Networks}
\author{
...
\textbf{Technical Contributions:} This paper proposes a general-purpose scaling architecture based on Elone semantics. The main contributions include:
\begin{enumerate}
...
\textbf{Implementation Snapshot (March 2026):} The current Rust codebase already implements the core consensus transaction model, the SDK signing and transaction builders, the off-chain engine/store boundary, and a low-level node RPC for serialized channel transaction submission. However, the implementation should still be regarded as pre-production: channel-factory lifecycle flows remain under stabilization, production PTLC adaptor-signature execution is incomplete, and the current repository does not yet contain a full PSTT workflow or the high-level wallet RPC surface discussed in earlier drafts.

\textbf{Keywords:} Payment Channel Networks, State Channels, Elone Protocol, UTXO Model, Finite State Machine, Formal Verification, Layer 2 Scaling
\end{abstract}
...
Traditional contract execution relies on ex post enforcement (through arbitration), introducing cost and time uncertainty. The Elone achieves ex ante enforcement through consensus rules:
\begin{align*}
\text{Traditional Mode: } & \text{Contract} \xrightarrow{\text{Dispute}} \text{Arbitration} \xrightarrow{\text{Judgment}} \text{Enforcement} \\
\text{The Elone: } & \text{State\_UTXO} \xrightarrow{\tau_{\text{settle}}} \text{Value\_Distribution} \quad (\text{deterministic execution})
\end{align*}
...
Blockchain system security is often described as ``trust minimization.'' The Elone further pursues \textbf{trust elimination}---making certain types of trust assumptions unnecessary through protocol design:
...
\toprule
\textbf{Trust Assumption} & \textbf{Traditional PCN} & \textbf{The Elone} & \textbf{Elimination Mechanism} \\
\midrule
...
The core insight of The Elone is: by pushing complexity down to the protocol layer, a simpler trust model can be achieved at the application layer.
