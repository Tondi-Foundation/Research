...
\begin{enumerate}
    ...
    \item \textbf{Constant-Time Classification and Bounded Verification}: We show how native transaction typing reduces transaction-family classification to $\mathcal{O}(1)$ while keeping validation cost bounded by explicit consensus resource limits.
    \item \textbf{Specification With Partial Implementation}: We provide a consensus-layer specification that is now partially realized in Rust across consensus, SDK, engine, store, and low-level RPC layers.
\end{enumerate}
...
\toprule
\textbf{Feature} & \textbf{Lightning} & \textbf{BIP-118} & \textbf{Elone} \\
\midrule
...
\begin{equation}
H_{\mathrm{NativeElone}}(t) \approx \mathrm{size}(\mathrm{State}_{\mathrm{curr}}) + \mathrm{size}(\mathrm{Fund}) \approx \mathcal{O}(1)
\end{equation}
...
\begin{tikzpicture}[
    ...
    % --- Right: Elone (Low Entropy) ---
    \node[font=\bfseries] at (6.5, 4.5) {Elone: $\mathcal{O}(1)$ Entropy};
    
    \node[state, fill=yellow!10] (fund) at (6.5, 3) {Fund UTXO};
    \node[active] (esn) at (6.5, 1) {$S_n$};
    
    \node[validator] (v_elone) at (8.5, 2) {Valid-\\ator};
    
    % Dependencies (Clean)
    \draw[arrow, blue, dashed] (esn) -- node[left, font=\tiny] {RefOp} (fund);
    \draw[arrow, blue] (v_elone) |- (esn);
    
    \node[below=0.2cm of esn, text=blue!70, font=\tiny] {Only check latest};

\end{tikzpicture}
\caption{Verification Causality Graph. Lightning validators (left) face linear complexity due to historical dependencies. Elone validators (right) only verify the latest state against the static Fund anchor.}
...
For Elone with $H = \mathcal{O}(1)$, only the current state and fund anchor are required. The recovery probability is $(1-p)^{O(1)} \approx 1$ for small $p$, independent of channel history length.
\end{proof}
