...
Table~\ref{tab:comparison} provides a systematic comparison between the original Eltoo protocol (which depends on BIP-118) and The Elone. For clarity, we refer to The Elone as \textbf{Elone} throughout this paper, reflecting its role as a successor that achieves Eltoo's design goals through consensus-native mechanisms rather than script-level simulation.

\begin{table}[htbp]
\centering
\caption{State Revocation Mechanisms: Original Eltoo vs.\ Elone (Proposed Architecture)}
\label{tab:comparison}
\small
\begin{tabularx}{\linewidth}{@{}p{2.2cm}XX@{}}
\toprule
\textbf{Feature} & \textbf{Original Eltoo (BIP-118)} & \textbf{Elone (This Paper)} \\
\midrule
...
\begin{itemize}
    \item \textbf{Abstraction Level:} Original Eltoo operates at the script layer, encoding state machine logic in spending conditions. Elone operates at the transaction type layer, where state transitions are first-class operations.
    
    \item \textbf{Security Model:} Original Eltoo's security depends on correct application-layer key management. Elone's security is enforced by consensus rules---applications cannot violate invariants even if buggy.
    
    \item \textbf{Deployment Path:} Original Eltoo requires a Bitcoin soft fork (BIP-118) with uncertain timeline. Elone is designed for new chains built with native support, avoiding political obstacles.
\end{itemize}

\subsection{Proposed Solution: UTXO-Native Semantics}

The defects enumerated above motivate our core architectural decision: rather than simulating Elone semantics through script-level tricks, we propose \textbf{embedding these semantics directly into the UTXO model and transaction type system}.
...
\begin{itemize}
    \item \textbf{Type Enforcement:} For all transactions $\tau$ in the Elone transaction set $\mathcal{T}_{\mathrm{Elone}}$, the inputs \rfc{must} be of type \code{ELONE\_STATE}. This is not a script-level check but a consensus-level type constraint.
    
    \item \textbf{Replay Isolation:} Because \code{ELONE\_STATE} UTXOs can only be spent by Elone update transactions, and standard UTXOs cannot be inputs to such transactions, replay attacks are \textit{physically impossible} at the type level---no key management discipline required.
\end{itemize}

\textbf{FSM vs.\ Arbitrary Recursion.} The Elone state machine follows strict monotonicity: each update transaction carries a sequence number $n$, and valid transitions require $n' > n$. This constraint is enforced by consensus:
...
The Elone is built upon a minimal set of axioms that capture the essential properties of sound payment channel operation. These axioms serve as the foundation for formal verification and guide implementation decisions.
...
The Elone is designed for deployment on GhostDAG-based consensus protocols~\cite{sompolinsky2015ghostdag, kaspa2021}. This choice is not incidental but architecturally motivated by the synergy between DAG properties and payment channel requirements.
...
\addlinespace
\textbf{Elone (Proposed)} & Sub-second (off-chain) & High (pooled via factories) & Consensus-enforced invariants \\
\bottomrule
\end{tabularx}
\end{table}
...
\textbf{Elone's Economic Advantage.} Our architecture enables larger, more dynamic channel factories through:
\begin{enumerate}
